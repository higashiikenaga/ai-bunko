...
from ainovel import arena, audio, bench, contest, translate, digest, images, inspiration, mood, odai, predict, sns, special, trends, views
...
def write_next_chapter(llm: BaseLLM, novel: Novel, cfg: dict) -> None:
    ...
    user = prompts.chapter_prompt(world, novel.characters, memory.build_context_bundle(), novel.last_tail(),
                                  ...
                                  + inspiration.prompt_block(novel, index))
...
def main(argv: list[str] | None = None) -> int:
    ...
    events += ["arena"] * int((cfg.get("arena") or {}).get("per_run", 1))  # アリーナ: 2つのモデルが同じお題で掌編を書く
        ...
        if event == "post":
            ...
            try:
                if event == "review":
                    ...
                    translate.translate_some(llm_for(llm, cfg, "sns"), int((cfg.get("i18n") or {}).get("per_run", 3)))
                elif event == "audio":
                    audio.make_one(cfg)
                elif event == "contest":
                ...
                print(f"  ✗ {({'review': 'レビュー', 'digest': 'ハイライト記事', 'trends': 'トレンド分析', 'image': '画像生成', 'audio': '朗読', 'translate': '翻訳', 'bench': 'ベンチ採点', 'arena': 'アリーナ'}).get(event, 'AI広場の投稿')}失敗: {e}")
                if isinstance(e, LLMError) and e.daily_quota:
                    state.mark_quota_exhausted()
                    stop_all = True
            finally:
